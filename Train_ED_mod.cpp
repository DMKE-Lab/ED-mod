...
{
	long long tid = (long long) tid_void;
...
void sgd()		//SGD training
{
	...
	for (int epoch=0; epoch<nepoch; epoch++)
	{
		...
		for (int batch = 0; batch<nbatches; batch++)
		{
			...
			for(int k = 0; k < THREADS_NUM; k ++){
				pthread_create(&threads[k], NULL, rand_sel, (void *)(long long)k);		//train
			}
			for(int k = 0; k < THREADS_NUM; k ++){
			...
			for(int k = 0; k < THREADS_NUM; k ++){
				pthread_create(&threads2[k], NULL, update_grad, (void *)(long long)k);		//update
			}
			for(int k = 0; k < THREADS_NUM; k ++){
...
void prepare()
{
    FILE* f1 = fopen("../data/entity.txt","r");
	FILE* f2 = fopen("../data/relation.txt","r");
	FILE* f3 = fopen("../data/timestamp.txt","r");
	FILE* f4 = fopen("../data/descriptions.txt","r");
	int x;
    ...
    cout<<"ICEWS_entity="<<7531<<endl;
    cout<<"ICEWS_relation="<<253<<endl;
    cout<<"ICEWS_time="<<287<<endl;
    cout<<"GDELT_entity="<<6172<<endl;
    cout<<"GDELT_relation="<<236<<endl;
    cout<<"GDELT_time="<<15<<endl;
    fclose(f_kb);
}
...
int main(int argc,char**argv)
{
	...
	int n_1 = 100;
	int n_w = 100;
    double rate = 0.0005;		//learning rate
    double margin = 0.5;		//loss margin
    int i;
